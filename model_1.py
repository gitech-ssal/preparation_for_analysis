...
class LinearRegression:
    def __init__(self, input_dim, output_dim,
        ...
        # 옵티마이저 설정 
        self.optimizer = self._get_optimizer(optimizer_type="adamw", lr=1e-2, weight_decay=1e-5)
        ...
        # 모델 디렉토리 설정 
        model_dir = "C:\\Users\\ssalt\\Documents\\ev_price_predict_project\\data\\train\\B_models\\a_model_1"
    ...
    def _setup_logging(self, model_dir, 
        ...
        if not continue_logging:
            ...
            # continue_logging이 True일 경우, 기존 핸들러가 없다면 새 핸들러 추가
            for handler in logger.handlers:
                if isinstance(handler, logging.FileHandler) and handler.baseFilename == log_file:
                    return  # 이미 해당 핸들러가 존재하면 아무 작업도 하지 않음
    ...
    def _initialize_weights(self, init_method):
        ...
        # nn.Sequential 내 각 레이어에 대해 초기화 적용
